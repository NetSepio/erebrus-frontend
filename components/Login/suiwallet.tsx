...
export const useSuiWallet = (setshowsignbutton) => {
  ...
  const { status, connected, account } = useWallet();
  ...
  async function handleSignMsg() {
    if (
      sendable &&
      wallet.account?.publicKey.length != 0 &&
      erebrusWallet == null
    ) {
      try {
        ...
        const verifyResult = await wallet.verifySignedMessage(
          result,
          Uint8Array.from(wallet.account!.publicKey)
        );
        if (!verifyResult) {
          console.log(
      ...
      try {
        ...
        const byteArray = Uint8Array.from(atob(result.signature), (c) =>
          c.charCodeAt(0)
        );

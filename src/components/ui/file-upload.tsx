    ...
    <div className="w-full" {...getRootProps()}>
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center">
          <p className="relative z-20 font-sans font-bold text-slate-300 dark:text-neutral-300 text-base">
            Upload file
          </p>
          <p className="relative z-20 font-sans font-normal text-slate-400 dark:text-neutral-400 text-base mt-2">
                ...
                >
                  <div className="flex justify-between w-full items-center gap-4">
                    <motion.p
                      ...
                      animate={{ opacity: 1 }}
                      layout
                      className="text-base text-slate-300 dark:text-neutral-300 truncate max-w-xs"
                    >
                      {file.name}
                    </motion.p>
                    <motion.p
                      ...
                      animate={{ opacity: 1 }}
                      layout
                      className="rounded-lg px-2 py-1 w-fit shrink-0 text-sm text-slate-300 dark:bg-neutral-800 dark:text-white shadow-input"
                    >
                      {(file.size / (1024 * 1024)).toFixed(2)} MB
                    </motion.p>
                  </div>

                  <div className="flex text-sm md:flex-row flex-col items-start md:items-center w-full mt-2 justify-between text-slate-400 dark:text-neutral-400">
                    <motion.p
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      layout
                      className="px-1 py-0.5 rounded-md bg-slate-700 dark:bg-neutral-800 "
                    >
                      {file.type}
                    </motion.p>
            ...
            {!files.length && (
              ...
              >
                {isDragActive ? (
                  ...
                  >
                    Drop it
                    <Upload className="h-4 w-4 text-slate-300 dark:text-neutral-400" />
                  </motion.p>
                ) : (
                  <Upload className="h-4 w-4 text-slate-300 dark:text-neutral-300" />
                )}
              </motion.div>
            )}
          ...
          return (
            <div
              ...
              className={`w-10 h-10 flex shrink-0 rounded-[2px] ${
                index % 2 === 0
                  ? "bg-slate-800 dark:bg-neutral-950"

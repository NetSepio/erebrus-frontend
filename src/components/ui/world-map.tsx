...
export default function WorldMap({ dots = [], nodes = [], lineColor = "#0ea5e9", onNodeClick }: MapProps) {
  ...
  const handleNodeClick = (lat: number, lng: number) => {
    if (onNodeClick) {
      onNodeClick(lat, lng)
    }
  }
      ...
      <svg ref={svgRef} viewBox="0 0 800 400" className="w-full h-full absolute inset-0 select-none">
        ...
        {nodes.map((node) => {
          ...
          return (
            <g
              key={`node-${node.id}`}
              onClick={() => handleNodeClick(node.lat, node.lng)}
            ...
            >
              <circle
                ...
                fill={node.status === "active" ? "#22c55e" : "#ef4444"}
                filter="url(#glow)"
              >

...
const EREBRUS_GATEWAY_URL = process.env.NEXT_PUBLIC_EREBRUS_BASE_URL || "https://gateway.erebrus.io"
...
export default function NodesData() {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof Node; direction: "asc" | "desc" } | null>(null)
    ...
    async function fetchNodes() {
      ...
      try {
        ...
        if (data && Array.isArray(data.payload)) {
          ...
          // Calculate stats
          const activeNodes = data.payload.filter((node: Node) => node.status === "active").length
          const uniqueRegions = new Set(data.payload.map((node: Node) => node.ipinfocountry || node.region)).size
  ...
  const filteredAndSortedNodes = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(
        (node) =>
          ...
          (node.ipinfocountry && node.ipinfocountry.toLowerCase().includes(term)) ||
    ...
    if (filters.region.length > 0) {
      result = result.filter((node) => filters.region.includes(node.ipinfocountry || node.region || "Unknown"))
    }
  ...
  return (
    <TooltipProvider>
      <div className="nodes-data-page pt-16 px-4 lg:px-20 bg-[#20253A]">
        <div className="text-2xl font-semibold text-gray-300 mb-8">Erebrus ÐVPN Nodes Data</div>

        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="bg-gradient-to-br from-blue-900/30 to-blue-800/10 border-blue-800/30">
            <CardHeader className="pb-2">
              <CardTitle className="text-gray-300 text-lg">Total Nodes</CardTitle>
            </CardHeader>
            <CardContent>
              {loading ? (
          ...
          <Card className="bg-gradient-to-br from-green-900/30 to-green-800/10 border-green-800/30">
            <CardHeader className="pb-2">
              <CardTitle className="text-gray-300 text-lg">Active Nodes</CardTitle>
            </CardHeader>
            <CardContent>
              {loading ? (
          ...
          <Card className="bg-gradient-to-br from-purple-900/30 to-purple-800/10 border-purple-800/30">
            <CardHeader className="pb-2">
              <CardTitle className="text-gray-300 text-lg">Regions Covered</CardTitle>
            </CardHeader>
            <CardContent>
              {loading ? (
              ...
              <DropdownMenuContent>
                <DropdownMenuItem onClick={() => toggleFilter("status", "active")}>
                  <div className="flex items-center">
                    <div
                      className={`w-4 h-4 mr-2 rounded-full ${filters.status.includes("active") ? "bg-green-500" : "bg-gray-300"}`}
                    ></div>
                    Active
                  </div>
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => toggleFilter("status", "inactive")}>
                  <div className="flex items-center">
                    <div
                      className={`w-4 h-4 mr-2 rounded-full ${filters.status.includes("inactive") ? "bg-red-500" : "bg-gray-300"}`}
                    ></div>
                    Inactive
                  </div>
                </DropdownMenuItem>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="max-h-60 overflow-y-auto">
                {uniqueRegions.map((region) => (
                  <DropdownMenuItem key={region} onClick={() => toggleFilter("region", region)}>
                    <div className="flex items-center">
                      <div
                        className={`w-4 h-4 mr-2 rounded-full ${filters.region.includes(region) ? "bg-blue-500" : "bg-gray-300"}`}
                      ></div>
                      {region}
                    </div>
                  </DropdownMenuItem>
        ...
        {(filters.status.length > 0 || filters.region.length > 0) && (
          <div className="flex flex-wrap gap-2 mb-4">
            {filters.status.map((status) => (
              <Badge key={status} variant="secondary" className="bg-blue-900/30 text-blue-200">
                Status: {status}
                <button className="ml-2 text-blue-200 hover:text-white" onClick={() => toggleFilter("status", status)}>
                  ×
                </button>
              </Badge>
            ))}
            ...
            {filters.region.map((region) => (
              <Badge key={region} variant="secondary" className="bg-purple-900/30 text-purple-200">
                Region: {region}
                <button
              ...
              <TableBody>
                {loading ? (
                  Array(5)
                    .fill(0)
                    .map((_, index) => (
                      <TableRow key={index} className="bg-gray-900/30 border-gray-800">
                        <TableCell>
                  ...
                  filteredAndSortedNodes.map((node) => (
                    ...
                    >
                      <TableCell className="font-medium text-white">{node.nodename || node.name}</TableCell>
                      <TableCell>
                        {node.status === "active" ? (
                      ...
                      <TableCell className="text-gray-300">{node.chainName || "Unknown"}</TableCell>
                      <TableCell>
                        <div className="flex items-center">
